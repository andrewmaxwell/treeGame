...
const OVERLAY_LIVING: ReadonlySet<Cell["type"]> = new Set<Cell["type"]>([
  "tree",
  "leaf",
  "flower",
  "fruit",
...
export function drawScene(
  ...
  for (const { cell, sx, sy } of treeCells) {
    ...
    if (overlay !== "none" && OVERLAY_LIVING.has(cell.type)) {
      ...
      const level =
        overlay === "water"
          ? cell.water / CELL_WATER_CAP
          : cell.energy / CELL_ENERGY_CAP;
      drawFilledHex(
        ctx,
        sx,
        sy,
  ...
  if (validPlacements.size > 0) {
    for (const [key, vpType] of validPlacements) {
      ...
      const stroke =
        vpType === "flower"
          ? "rgba(255,170,176,0.40)"
          : underground
            ? "rgba(255,212,150,0.7)"
            : "rgba(190,150,100,0.20)";
      hexPath(ctx, sx, sy, r * 0.92);
      ctx.fillStyle =
        vpType === "flower"
          ? "rgba(255,170,176,0.08)"
          : underground
            ? "rgba(110,70,30,0.30)"
            : "rgba(160,115,65,0.04)";
      ctx.fill();
