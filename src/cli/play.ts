...
function countType(
  g: Headless,
  pred: (c: { type: string; r: number; q: number; health: number }) => boolean,
) {
  let n = 0;
...
function leafCount(g: Headless) {
  return countType(g, (c) => c.type === "leaf");
}
...
function tallGrower(g: Headless) {
  const s = g.season;
  if (s === "winter") {
    if (rootDepth(g) < 16) extendDown(g, 2);
    return;
  }
  if (s === "fall") return;
...
function groundCrawler(g: Headless) {
  ...
  if (!isFinite(minQ)) g.place(0, sr - 1, "branch");
    ...
    for (let k = 0; k < 4; k++) {
      g.place(maxQ + 1 + k, sr - 1, "branch");
      g.place(minQ - 1 - k, sr - 1, "branch");
    }
...
function run(
  ...
  for (let i = 0; i < years * 4; i++) {
    ...
    const bands = g.bands();
    const flowerNote =
      g.season === "spring" || rep.season === "spring"
    ...
    console.log(
      `        bands top[${band(bands.top)}] mid[${band(bands.mid)}] low[${band(bands.low)}] roots[${band(bands.roots)}]`,
    );
    const repro = rep.summaryEvents.filter((e) =>
      /bloom|fruit|seed|Harvest/i.test(e),
    );
    if (repro.length) console.log(`        ${repro.join("  ")}`);
    if (rep.newlyCompleted.length)
      console.log(`        🏅 ${rep.newlyCompleted.join(", ")}`);
    if (g.livingCount() === 0) {
...
function sweep(
  ...
  for (const seed of seeds) {
    ...
    scores.push(g.score);
    console.log(
      `  seed ${String(seed).padStart(5)}: score ${String(g.score).padStart(3)} cells ${String(g.livingCount()).padStart(3)} ${died ? `DIED y${died}` : `alive y${g.year}`}`,
    );
  }
  const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
  console.log(
    `  → avg score ${avg.toFixed(1)}, min ${Math.min(...scores)}, max ${Math.max(...scores)}`,
  );
}
