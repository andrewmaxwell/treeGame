...
function spendOnWood(g: Headless) {
  ...
  while (g.remaining >= 1 && guard++ < 400) {
    ...
    // widen
    if (g.remaining >= 1)
      for (const [k, ty] of g.validPlacements("branch")) {
        if (ty !== "tree") continue;
...
for (let y = 1; y <= 8; y++) {
  for (let s = 0; s < 4; s++) {
    ...
    if (g.season === "fall") {
      ...
      const lw = leaves.length
        ? (leaves.reduce((a, c) => a + c.water, 0) / leaves.length).toFixed(2)
        : "—";
      console.log(
        `${String(g.year).padStart(3)}  ${season.padEnd(7)} ${String(g.livingCount()).padStart(5)}  ${String(Math.round(g.banked)).padStart(6)}  ${String(lw).padStart(8)}        ${leaves.length}`,
      );
    }
  }
}
