...
export class Headless {
  ...
  get budget() {
    return this.planning.energyAvailable;
  }
  get spent() {
    return this.planning.energySpent;
  }
  get remaining() {
    return this.planning.energyAvailable - this.planning.energySpent;
  }
  get banked() {
    return bankedEnergy(this.game.cells);
  }
  get weather(): SeasonWeather {
  ...
  validPlacements(mode: PlacementMode) {
    return getValidPlacements(mode, this.game, this.planning);
  }
  ...
  advance(): AdvanceReport {
    ...
    const ctx: GoalContext = {
      ...
      grewFlowerThisTurn: [...committed.cells.values()].some(
        (c) => c.type === "flower",
      ),
    };
    ...
    return {
      ...
      year: weather.year,
      weather:
        weatherHeadline(weather).label +
        (weather.isDrought ? " (drought)" : ""),
      bankedBefore,
  ...
  goal(): string | null {
    return currentGoal(this.game.goals)?.goal ?? null;
  }
  ...
  bands() {
    const above: Cell[] = [],
      roots: Cell[] = [];
    for (const c of this.game.cells.values()) {
      if (c.type === "soil" || c.type === "rock" || c.type === "deadwood")
        continue;
      if (c.r >= surfaceR(c.q)) roots.push(c);
      else above.push(c);
    }
    ...
    const agg = (cs: Cell[]) =>
      cs.length === 0
        ...
        : {
            n: cs.length,
            water: +(cs.reduce((a, c) => a + c.water, 0) / cs.length).toFixed(
              1,
            ),
            energy: +(cs.reduce((a, c) => a + c.energy, 0) / cs.length).toFixed(
              1,
            ),
            health: +(cs.reduce((a, c) => a + c.health, 0) / cs.length).toFixed(
              2,
            ),
  ...
  flowerDiagnosis() {
    ...
    for (const c of this.game.cells.values()) {
      ...
      aboveGroundWood++;
      const hasUp =
        this.game.cells.get(hexKey(c.q, c.r - 1))?.type === "tree" ||
        this.game.cells.get(hexKey(c.q + 1, c.r - 1))?.type === "tree";
      if (!hasUp) {
        tips++;
        if (c.health > 0.6) healthyTips++;
      }
    }
    return {
      valid,
  ...
  render(opts: { soil?: boolean; pad?: number } = {}): string {
    ...
    const cells = [...this.game.cells.values()].filter(
      (c) => c.type !== "rock" && (opts.soil || c.type !== "soil"),
    );
