...
const top = (g: Headless) =>
  [...g.game.cells.values()]
    .filter((c) => c.type === "tree")
    .sort((a, b) => a.r - b.r)[0];
const hgt = (g: Headless) => {
  const t = top(g);
...
console.log(
  `spring budgets by year: ${springBudgets.join(" ")} | final cells ${g.livingCount()} score ${g.score}`,
);
