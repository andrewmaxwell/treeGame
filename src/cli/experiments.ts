...
function buildTrunk(height: number, width: number, leaves: number): GameState {
  ...
  for (let d = 0; d <= 4; d++)
    for (const c of cols)
      cells.set(hexKey(c, S + d), tree(c, S + d, { water: 8 }));
  for (let d = -1; d <= 5; d++)
    for (let c = -3; c <= 3; c++) {
  ...
  for (let h = 1; h <= height; h++)
    for (const c of cols)
      cells.set(hexKey(c, S - h), tree(c, S - h, { water: 5 }));
...
function bandHealth(state: GameState) {
  ...
  const grp = (pred: (c: Cell) => boolean) => {
    ...
    if (!cs.length) return "  —";
    const avg = (f: (c: Cell) => number) =>
      cs.reduce((a, c) => a + f(c), 0) / cs.length;
...
function topStats(state: GameState) {
  ...
  for (const c of state.cells.values())
    if (c.type === "tree" && c.r < minR && c.r < surfaceR(c.q)) minR = c.r;
  for (const c of state.cells.values())
    if (c.type === "tree" && c.r === minR) topTrunkW = c.water;
...
console.log(
  "Conduction experiment — per-band health/water/energy at end of a summer season",
);
console.log(
  "Can a TALL tree keep leaf + trunk + roots all healthy? Need leaf water>3 for h1.\n",
);
console.log(
  "height width leaves | leaves               canopyWood          midTrunk            roots",
);
