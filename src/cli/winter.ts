...
function extendDown(g: Headless, n: number) {
  for (let i = 0; i < n; i++) {
    const d = deepestRoot(g); if (!d) return
    if (!g.place(d.q, d.r + 1, 'branch') && !g.place(d.q + 1, d.r + 1, 'branch') && !g.place(d.q - 1, d.r + 1, 'branch')) return
  }
}
function extendUp(g: Headless, n: number) {
  for (let i = 0; i < n; i++) {
    const t = trunkTop(g); if (!t) return
    if (!g.place(t.q, t.r - 1, 'branch') && !g.place(t.q + 1, t.r - 1, 'branch') && !g.place(t.q - 1, t.r - 1, 'branch')) return
  }
}
...
function countType(g: Headless, type: string) { return cells(g).filter((c) => c.type === type).length }
function dryWood(g: Headless) {  // above-ground wood the OLD rule would have been killing
  return cells(g).filter((c) => c.type === 'tree' && c.r < surfaceR(c.q) && c.water <= 0.5).length
}

const g = new Headless(2024)
console.log('year  season  living  deadwood  dryWood  topBand(health/min)  woodAvgHealth')
for (let y = 1; y <= 5; y++) {
  for (let s = 0; s < 4; s++) {
    ...
    if (season !== 'winter' && season !== 'fall') {
      if (g.year <= 4) {
        extendDown(g, 2); extendUp(g, 1); widen(g, 2)
      }
    ...
    g.advance()
    const b = g.bands().top
    const woodCells = cells(g).filter((c) => c.type === 'tree' && c.r < surfaceR(c.q))
    const avgH = woodCells.length ? (woodCells.reduce((a, c) => a + c.health, 0) / woodCells.length).toFixed(2) : '—'
    console.log(
