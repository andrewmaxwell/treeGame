...
function sample(): GameState {
  ...
  return {
    ...
    rngSeed: 111,
    worldSeed: 222,
    goals: { completed: ["first-leaf", "survive-season"], peakCells: 14 },
  };
}
...
describe("save round-trip", () => {
  it("serialize → deserialize preserves all gameplay state", () => {
    ...
    expect(restored.goals).toEqual({
      completed: ["first-leaf", "survive-season"],
      peakCells: 14,
    });

    expect(restored.cells.size).toBe(3);
    expect(restored.cells.get(hexKey(0, 0))).toEqual(
      game.cells.get(hexKey(0, 0)),
    );
