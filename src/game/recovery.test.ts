...
function advance(
  game: GameState,
  plan: (g: GameState, p: PlanningState) => PlanningState,
): GameState {
  ...
  const budget =
    game.season === "spring" ? Math.max(banked, SPRING_VIGOR) : banked;
...
function trunkTop(game: GameState): { q: number; r: number } {
  ...
  for (const c of game.cells.values())
    if (c.type === "tree" && c.r < best.r) best = { q: c.q, r: c.r };
  return best;
}
