...
describe("evaluateGoals", () => {
  it("completes the first leaf + survive-season together on a leafy first season", () => {
    ...
    const { progress, newlyCompleted } = evaluateGoals(
      fresh,
      ctx({ cells: c, livingCells: 2, peakCells: 2 }),
    );
  ...
  it("does not re-complete an already-completed milestone", () => {
    ...
    const again = evaluateGoals(
      after.progress,
      ctx({ cells: c, livingCells: 2 }),
    );
  ...
  it("uses peak cells, not current, so winter shrinkage does not block size goals", () => {
    ...
    const { newlyCompleted } = evaluateGoals(
      fresh,
      ctx({ livingCells: 4, peakCells: 12 }),
    );
  ...
  it("shed-leaves only fires when leaves were shed in a fall turn", () => {
    expect(
      evaluateGoals(
        fresh,
        ctx({
          seasonSimulated: "spring" as Season,
          shedThisTurn: true,
          livingCells: 1,
        }),
      ).newlyCompleted.map((m) => m.id),
    ).not.toContain("shed-leaves");
    expect(
      evaluateGoals(
        fresh,
        ctx({
          seasonSimulated: "fall" as Season,
          shedThisTurn: true,
          livingCells: 1,
        }),
      ).newlyCompleted.map((m) => m.id),
    ).toContain("shed-leaves");
  });
  ...
  it("survive-winter requires a winter season", () => {
    expect(
      evaluateGoals(
        fresh,
        ctx({ seasonSimulated: "winter" as Season, livingCells: 3 }),
      ).newlyCompleted.map((m) => m.id),
    ).toContain("survive-winter");
  });
  ...
  it("survive-drought fires only when a drought season is survived", () => {
    expect(
      evaluateGoals(
        fresh,
        ctx({ droughtThisSeason: false, livingCells: 3 }),
      ).newlyCompleted.map((m) => m.id),
    ).not.toContain("survive-drought");
    expect(
      evaluateGoals(
        fresh,
        ctx({ droughtThisSeason: true, livingCells: 3 }),
      ).newlyCompleted.map((m) => m.id),
    ).toContain("survive-drought");
  });
  ...
  it("survive-storm fires only on a storm season with zero cells lost", () => {
    ...
    expect(
      evaluateGoals(
        fresh,
        ctx({ stormThisSeason: true, stormCellsLost: 4, livingCells: 3 }),
      ).newlyCompleted.map((m) => m.id),
    ).not.toContain("survive-storm");
    // No storm at all → no completion.
    expect(
      evaluateGoals(
        fresh,
        ctx({ stormThisSeason: false, stormCellsLost: 0, livingCells: 3 }),
      ).newlyCompleted.map((m) => m.id),
    ).not.toContain("survive-storm");
    ...
    expect(
      evaluateGoals(
        fresh,
        ctx({ stormThisSeason: true, stormCellsLost: 0, livingCells: 3 }),
      ).newlyCompleted.map((m) => m.id),
    ).toContain("survive-storm");
  });
});
...
describe("currentGoal / completedMilestones", () => {
  ...
  it("completedMilestones returns them in completion order", () => {
    ...
    expect(completedMilestones(p).map((m) => m.id)).toEqual([
      "survive-season",
      "first-leaf",
    ]);
  });
