  ...
  it('guarantees Year 1 some rain (gentle start)', () => {
    for (const season of ['spring', 'summer', 'fall', 'winter'] as const) {
      for (let seed = 0; seed < 50; seed++) {
        expect(rainTickCount(generateWeather(season, 1, seed))).toBeGreaterThan(0)
      }
    }
  })
})
...
describe('weatherHeadline', () => {
  it('flags drought and winter frost regardless of rain', () => {
    ...
    if (drought.isDrought) expect(weatherHeadline(drought).label).toBe('Drought')
    expect(weatherHeadline(generateWeather('winter', 1, 0)).label).toBe('Frost')
  })
})
