...
function canExchangeWater(a: Cell, b: Cell): boolean {
  ...
  if (a.type === "soil" && isLivingWood(b.type) && isUnderground(b))
    return true;
  if (b.type === "soil" && isLivingWood(a.type) && isUnderground(a))
    return true;
  if (isLivingWood(a.type) && isLivingWood(b.type)) return true;
  if (isLivingWood(a.type) && isTerminalType(b.type)) return true;
  if (isLivingWood(b.type) && isTerminalType(a.type)) return true;
  if (a.type === "deadwood" && (isLivingWood(b.type) || b.type === "deadwood"))
    return true;
  if (b.type === "deadwood" && (isLivingWood(a.type) || a.type === "deadwood"))
    return true;
  if (
    (a.type === "ground water" &&
      (isLivingWood(b.type) || b.type === "deadwood" || b.type === "soil")) ||
    (b.type === "ground water" &&
      (isLivingWood(a.type) || a.type === "deadwood" || a.type === "soil"))
  )
    return true;
...
function canExchangeEnergy(a: Cell, b: Cell): boolean {
  if (isLivingWood(a.type))
    return isLivingWood(b.type) || isTerminalType(b.type);
  if (isLivingWood(b.type)) return isTerminalType(a.type);
  return false;
}
...
function buildWork(state: GameState): Map<string, Cell> {
  ...
  for (const cell of state.cells.values()) {
    ...
    for (const [dq, dr] of HEX_NEIGHBORS) {
      ...
      if (tc?.type === "soil" || tc?.type === "ground water") {
        work.set(key, { ...tc });
...
function autoLeafFill(
  ...
  for (let pass = 0; pass < 40; pass++) {
    ...
    for (const k of candidates) {
      const [q, r] = k.split(",").map(Number);
      const gen =
        (light.get(k) ?? 0) * intensity * PHOTO_COEFF * heightLightFactor(q, r);
      if (gen > AUTO_LEAF_MIN_GEN) {
        work.set(k, mkLeaf(q, r));
        added.add(k);
        addedThisPass = true;
      }
    }
...
export function growAutoLeaves(
  state: GameState,
  weather: SeasonWeather,
): GameState {
  return {
    ...state,
    cells: autoLeafFill(state, weather.sunAngleDeg, weather.intensity).cells,
  };
}

// Keys auto-leaves would occupy for a given state + season light — for the planning preview.
export function autoLeafPreview(
  state: GameState,
  sunAngleDeg: number,
  intensity: number,
): Set<string> {
...
export function photosynthesize(
  state: GameState,
  ...
  for (const [key, cell] of state.cells) {
    ...
    const gain =
      ...
      intensity *
      PHOTO_COEFF *
      heightLightFactor(cell.q, cell.r) *
      photoWaterFactor(cell.water);
    if (gain <= 0) continue;
    work.set(key, {
      ...cell,
      energy: Math.min(cell.energy + gain, CELL_ENERGY_CAP),
    });
  }
...
function metabolize(state: GameState, mult: number): GameState {
  ...
  for (const [key, cell] of state.cells) {
    ...
    if (cell.type === "leaf" || cell.type === "fruit")
      w *= stomataFactor(cell.water);
    newCells.set(key, {
      ...cell,
...
export function absorbWater(state: GameState, rng: RNG): GameState {
  ...
  for (const cell of state.cells.values()) {
    if (!isLivingWood(cell.type)) continue;
    if (!isUnderground(cell)) continue;
    ...
    for (const [dq, dr] of HEX_NEIGHBORS) {
      ...
      if (
        work.get(nKey)?.type === "soil" ||
        work.get(nKey)?.type === "ground water"
      )
        soilNeighborKeys.push(nKey);
    }

    // Shuffle to remove directional bias
    for (let i = soilNeighborKeys.length - 1; i > 0; i--) {
      const j = Math.floor(rng() * (i + 1));
      [soilNeighborKeys[i], soilNeighborKeys[j]] = [
        soilNeighborKeys[j],
        soilNeighborKeys[i],
      ];
    }
    ...
    for (const nKey of soilNeighborKeys) {
      ...
      if (soil.type === "ground water") {
        amount = Math.min(inBudget, CELL_WATER_CAP - treeCell.water);
      } else {
        amount = Math.min(
          soil.water * 0.05,
          inBudget,
          CELL_WATER_CAP - treeCell.water,
          soil.water,
        );
      }
...
function diffuse(state: GameState, rng: RNG, cfg: DiffuseConfig): GameState {
  ...
  for (let i = 0; i < pairsA.length; i++) {
    ...
    if (recv.type === "ground water") continue;
    const flow = Math.min(
...
export function diffuseWater(state: GameState, rng: RNG): GameState {
  return diffuse(state, rng, {
    get: (c) => (c.type === "ground water" ? GROUND_WATER_CAP : c.water),
    set: (c, v) => (c.type === "ground water" ? c : { ...c, water: v }),
    canExchange: canExchangeWater,
    capacity: (c) =>
      c.type === "ground water" ? Number.POSITIVE_INFINITY : waterCap(c),
    budget: (c) =>
      c.type === "deadwood" ? 0.3 : c.type === "ground water" ? 100.0 : 2.0, // deadwood: capillary only
  });
}
...
export function matureFruit(state: GameState): GameState {
  ...
  for (const [key, cell] of state.cells) {
    ...
    if (cell.water >= FRUIT_FED_WATER)
      m = Math.min(FRUIT_RIPE, m + FRUIT_RIPEN_RATE);
    else if (cell.water < FRUIT_THIRSTY_WATER) m -= FRUIT_DECLINE_RATE;
    if (m <= 0)
      work.delete(key); // aborted — dropped, no seed
...
function spreadRot(state: GameState, _rng: RNG): GameState {
  return state;
}
...
function winterFrost(state: GameState): GameState {
  ...
  for (const [key, cell] of state.cells) {
    if (
      ...
      if (cell.type === "leaf")
        depositResorb(work, cell, cell.energy * LEAF_FROST_RESORB);
      work.delete(key);
    } else if (
      cell.type === "tree" &&
      cell.age === 0 &&
      cell.r < surfaceR(cell.q)
    ) {
...
function depositResorb(
  work: Map<string, Cell>,
  leaf: Cell,
  amount: number,
): void {
  if (amount <= 0) return;
...
export function ripenFruit(state: GameState): {
  state: GameState;
  harvested: number;
} {
  const work = new Map(state.cells);
  ...
  return {
    state: { ...state, cells: work, score: state.score + harvested },
    harvested,
  };
}
...
function runTick(
  ...
  if (
    storm &&
    tick >= storm.startTick &&
    tick < storm.startTick + storm.ticks
  ) {
    const r = applyStorm(s, rng, storm.severity);
    s = r.state;
    if (r.cellsLost > 0)
      brk = { cellsLost: r.cellsLost, severity: storm.severity };
  }
