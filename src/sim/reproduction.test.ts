...
import {
  FRUIT_START_MATURITY, FRUIT_RIPEN_RATE, FRUIT_DECLINE_RATE, FLOWER_SET_HEALTH,
} from './cells'
...
describe('setFruit', () => {
  ...
  it('a flower at/below the set-health bar drops (no fruit)', () => {
    const s = setFruit(makeState([mk(0, -1, 'flower', { health: FLOWER_SET_HEALTH })]))
...
describe('matureFruit', () => {
  it('a well-fed fruit (water ≥ 2) ripens by the ripen rate', () => {
    const s = matureFruit(makeState([mk(0, -1, 'fruit', { water: 5, maturity: 0.5 })]))
    expect(s.cells.get(hexKey(0, -1))!.maturity).toBeCloseTo(0.5 + FRUIT_RIPEN_RATE)
  })

  it('maturity never exceeds 1.0', () => {
    const s = matureFruit(makeState([mk(0, -1, 'fruit', { water: 5, maturity: 0.99 })]))
    expect(s.cells.get(hexKey(0, -1))!.maturity).toBe(1.0)
  })

  it('a thirsty fruit (water < 1) regresses by the decline rate', () => {
    const s = matureFruit(makeState([mk(0, -1, 'fruit', { water: 0.5, maturity: 0.5 })]))
    expect(s.cells.get(hexKey(0, -1))!.maturity).toBeCloseTo(0.5 - FRUIT_DECLINE_RATE)
  })

  it('holds steady at intermediate water (1 ≤ water < 2)', () => {
    const s = matureFruit(makeState([mk(0, -1, 'fruit', { water: 1.5, maturity: 0.5 })]))
    expect(s.cells.get(hexKey(0, -1))!.maturity).toBe(0.5)
  })

  it('aborts (drops) when sustained thirst drives maturity to ≤ 0', () => {
    let s = makeState([mk(0, -1, 'fruit', { water: 0, maturity: FRUIT_START_MATURITY })])
...
describe('ripenFruit', () => {
  it('a ripe fruit yields one seed and drops', () => {
    const { state, harvested } = ripenFruit(makeState([mk(0, -1, 'fruit', { maturity: 1.0 })], { score: 2 }))
  ...
  it('an unripe fruit drops with no seed', () => {
    const { state, harvested } = ripenFruit(makeState([mk(0, -1, 'fruit', { maturity: 0.8 })], { score: 2 }))
  ...
  it('scores one seed per ripe fruit across many', () => {
    const { harvested } = ripenFruit(makeState([
...
describe('fruit structural load', () => {
  it('a fruit on a branch raises the limb stress vs the same branch bare', () => {
    ...
    const bare = computeStructure(new Map(arm.map((c) => [hexKey(c.q, c.r), c])))
    const withFruit = computeStructure(new Map(
