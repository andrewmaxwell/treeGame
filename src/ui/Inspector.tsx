...
function status(cell: Cell): { text: string; tone: "good" | "warn" | "bad" } {
  if (cell.type === "deadwood")
    return { text: "Dead wood — still bears load", tone: "warn" };
...
export function Inspector({
  ...
  const isTerrain =
    cell.type === "soil" ||
    cell.type === "rock" ||
    cell.type === "ground water";
  const waterCap = cell.type === "soil" ? SOIL_WATER_CAP : CELL_WATER_CAP;
  const st = status(cell);

  const typeName =
    cell.type === "tree" ? woodLabel(cell) : TYPE_LABEL[cell.type];
  const canPrune = !isTerrain && !removesAll && (cost === 0 || affordable);

  const pruneLabel =
    removalCount > 1 ? `Prune — ${removalCount} cells removed` : "Prune";

  function handlePrune() {
    if (seversCanopy && !confirming) {
      setConfirming(true);
      return;
    }
    onPrune();
  ...
  return (
    <div className={styles.panel} onClick={(e) => e.stopPropagation()}>
      ...
      {!isTerrain && (
        <div className={`${styles.status} ${styles[st.tone]}`}>{st.text}</div>
      )}
      ...
      <div className={styles.rows}>
        ...
        {cell.type !== "soil" && (
          <Row
            label="Energy"
            value={`${cell.energy.toFixed(1)} / ${CELL_ENERGY_CAP}`}
          />
        )}
        {!isTerrain && (
          <Row label="Health" value={`${Math.round(cell.health * 100)}%`} />
        )}
        {!isTerrain && cell.rot > 0 && (
          <Row label="Rot" value={`${Math.round(cell.rot * 100)}%`} />
        )}
        {stress !== undefined && (
          <Row
            label="Load stress"
        ...
        {stress !== undefined && stress > STRESS_WARN && (
          <p className={styles.hint}>
            A one-sided limb bends hardest here. Widen the trunk{" "}
            <em>at this row</em>, shorten the heavy branch, or balance it with
        ...
        {!isTerrain && (
          <Row
            label="Age"
            value={`${cell.age} ${cell.age === 1 ? "season" : "seasons"}`}
          />
        )}
      </div>
      ...
      {!isTerrain && cell.type === "leaf" && (
        <p className={styles.hint}>
          Leaves grow and fall on their own — no need to prune them.
        </p>
      )}
        ...
        <>
          {removesAll && (
            <div className={styles.warnLine}>
              This is your whole tree — prune something smaller, or "Plant a new
              seed" to start over.
            </div>
          )}
          {seversCanopy && confirming && (
            <div className={styles.warnLine}>
              This removes your whole canopy. Tap again to confirm.
            </div>
          )}
          <button
            ...
            title={
              removesAll
                ...
                : !canPrune
                  ? "Not enough energy to seal the wound"
                  : undefined
            }
          >
